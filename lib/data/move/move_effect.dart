...
class MoveEffect {
  ...
  static void initialize() {
    ...
    recoilFraction = MoveEffect("recoil_fraction",
        onDoDamage:
            (state, user, target, damage, extra) {
      final recoil = max(1, damage * extra ~/ 100);
      user.individual.hp -= recoil;
      print('Took $recoil in recoil damage!');
            }); // Param = % recoil
    changeStats = MoveEffect("change_stats"); // Param = encoded stat change
    inflictTarget =
        MoveEffect("inflict_target"); // Param = id of status condition
    inflictSelf = MoveEffect("inflict_self"); // Param = id of status condition
    affectsEverything = MoveEffect("affects_everything");
